                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            <div class="form-row">
                            <!-- 级联表的字段 -->
                                    <div class="form-group col-md-6 aaaaaa banzhuren">
                                        <label>班主任</label>
                                        <div>
                                            <select style="width: 450px" id="banzhurenSelect" name="banzhurenSelect"
                                                    class="selectpicker form-control"  data-live-search="true"
                                                    title="请选择" data-header="请选择" data-size="5" data-width="650px">
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group col-md-6 banzhuren">
                                        <label>班主任姓名</label>
                                        <input style="width: 450px" id="banzhurenName" name="banzhurenName" class="form-control"
                                               placeholder="班主任姓名" readonly>
                                    </div>
                                    <div class="form-group col-md-6 banzhuren">
                                        <label>班主任手机号</label>
                                        <input style="width: 450px" id="banzhurenPhone" name="banzhurenPhone" class="form-control"
                                               placeholder="班主任手机号" readonly>
                                    </div>
                                    <div class="form-group col-md-6 banzhuren">
                                        <label>班主任头像</label>
                                        <img id="banzhurenPhotoImg" src="" width="100" height="100">
                                    </div>
                            <!-- 当前表的字段 -->
                                    <input id="updateId" name="id" type="hidden">
                                <input id="banzhurenId" name="banzhurenId" type="hidden">
                                    ...
                                    <div class="form-group col-md-6 kechengAddressDiv">
                                        <label>上课地点</label>
                                        <input style="width: 450px" id="kechengAddress" name="kechengAddress" class="form-control"
                                               placeholder="上课地点">
                                    </div>
                                    <div class="form-group col-md-6 kechengNumberDiv">
                                        ...
                                        <input style="width: 450px" id="kechengNumber" name="kechengNumber" class="form-control"
                                               onchange="kechengNumberChickValue(this)"  placeholder="最大选课人数">
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            let data = {};
            getContent();
            if(window.sessionStorage.getItem('role') != '班主任'){//当前登录用户不为这个
                if($("#banzhurenId") !=null){
                    var banzhurenId = $("#banzhurenId").val();
                    if(banzhurenId == null || banzhurenId =='' || banzhurenId == 'null'){
                        alert("班主任不能为空");
                        return;
                    }
                }
    ...
    // 查询列表
        ...
        function banzhurenSelect() {
            //填充下拉框选项
            http("banzhuren/page?page=1&limit=100&sort=&order=", "GET", {}, (res) => {
                if(res.code == 0){
                    banzhurenOptions = res.data.list;
                }
            });
        }

        function banzhurenSelectOne(id) {
            http("banzhuren/info/"+id, "GET", {}, (res) => {
                if(res.code == 0){
                ruleForm = res.data;
                banzhurenShowImg();
                banzhurenShowVideo();
    ...
    <!-- 初始化当前表的下拉框 -->
        ...
        function initializationbanzhurenSelect() {
            var banzhurenSelect = document.getElementById('banzhurenSelect');
            if(banzhurenSelect != null && banzhurenOptions != null  && banzhurenOptions.length > 0 ) {
                for (var i = 0; i < banzhurenOptions.length; i++) {
                        banzhurenSelect.add(new Option(banzhurenOptions[i].banzhurenName, banzhurenOptions[i].id));
                }

                $("#banzhurenSelect").change(function(e) {
                        banzhurenSelectOne(e.target.value);
                });
            }
    ...
    function setSelectOption() {
        ...
        <!--  级联表的下拉框回显  -->
            ...
            if(banzhurenSelect != null && banzhurenOptions != null  && banzhurenOptions.length > 0 ) {
                for (var i = 0; i < banzhurenOptions.length; i++) {
                    if (banzhurenOptions[i].id == ruleForm.banzhurenId) {//下拉框value对比,如果一致就赋值汉字
                        banzhurenSelect.options[i+1].selected = true;
                        $("#banzhurenSelect" ).selectpicker('refresh');
                    }
                }
            }
    }
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            ...
            messages: {
                ...
                kechengShijian: "上课时间不能为空",
                kechengAddress: "上课地点不能为空",
                kechengNumber: "最大选课人数不能为空",
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#banzhurenId").val(ruleForm.banzhurenId);
        ...
        $("#kechengAddress").val(ruleForm.kechengAddress);
        $("#kechengNumber").val(ruleForm.kechengNumber);
        $("#kechengContent").val(ruleForm.kechengContent);

    }
    <!--  级联表的数据回显  -->
    function banzhurenDataBind(){

                    <!-- 把id赋值给当前表的id-->
        $("#banzhurenId").val(ruleForm.id);

        $("#banzhurenName").val(ruleForm.banzhurenName);
        $("#banzhurenPhone").val(ruleForm.banzhurenPhone);
        $("#banzhurenEmail").val(ruleForm.banzhurenEmail);
    }
    ...
    function banzhurenShowImg() {
        $("#banzhurenPhotoImg").attr("src",ruleForm.banzhurenPhoto);
    }
    ...
    function banzhurenShowVideo() {
        $("#banzhurenPhotoV").attr("src",ruleForm.banzhurenPhoto);
    }
    ...
    function readonly() {
		...
		else if (window.sessionStorage.getItem('role') == '班主任') {
            // $(".aaaaaa").remove();
            $(".banzhuren").remove();//删除当前用户的信息
        }
        else{
            // alert("未知情况.......");
