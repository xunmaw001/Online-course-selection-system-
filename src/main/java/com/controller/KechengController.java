    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(kecheng !=null){
            ...
            BeanUtils.copyProperties( kecheng , view );//把实体数据重构到view中
                ...
                //级联表
                BanzhurenEntity banzhuren = banzhurenService.selectById(kecheng.getBanzhurenId());
                if(banzhuren != null){
                    BeanUtils.copyProperties( banzhuren , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setBanzhurenId(banzhuren.getId());
                }
            //修改对应字典表字段
    ...
    */
    ...
    public R save(@RequestBody KechengEntity kecheng, HttpServletRequest request){
        ...
        if(false)
            ...
            kecheng.setBanzhurenId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<KechengEntity> queryWrapper = new EntityWrapper<KechengEntity>()
            .eq("banzhuren_id", kecheng.getBanzhurenId())
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            KechengEntity kechengEntity = new KechengEntity();
//                            kechengEntity.setBanzhurenId(Integer.valueOf(data.get(0)));   //班主任 要改的
...
//                            kechengEntity.setKechengShijian(data.get(0));                    //上课时间 要改的
//                            kechengEntity.setKechengAddress(data.get(0));                    //上课地点 要改的
//                            kechengEntity.setKechengNumber(Integer.valueOf(data.get(0)));   //最大选课人数 要改的
        ...
        KechengEntity kecheng = kechengService.selectById(id);
            if(kecheng !=null){
                ...
                //级联表
                    BanzhurenEntity banzhuren = banzhurenService.selectById(kecheng.getBanzhurenId());
                if(banzhuren != null){
                    BeanUtils.copyProperties( banzhuren , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setBanzhurenId(banzhuren.getId());
                }
                //修改对应字典表字段
                dictionaryService.dictionaryConvert(view, request);
